...
| Git/policy fallback | All editors | Enforcement at review/commit time. | `tellur policy check`, local PR reports, and portable `refs/notes/ai` catch gaps in editor APIs. CI team reports intersect notes with the actual per-commit Git additions and mark uncovered lines unknown instead of assuming 0% AI. |
