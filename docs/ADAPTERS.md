...
| Git/policy fallback | All editors | Enforcement at review/commit time. | `tellur policy check`, local PR reports, and portable `refs/notes/ai` catch gaps in editor APIs. Normal note export requires exact commit-blob matches and intersects ranges with the commit additions. Explicit missed-capture recovery is stored as `claimed`; CI reports evidence strength and mark uncovered lines unknown instead of assuming 0% AI. |
