...
- Prompt-like fields (`message`, `prompt`, `text`, `content`) are hashed rather
  than stored as raw text in Codex/Copilot adapter payloads.
...
- Cursor and Copilot imports should be validated against representative real
  exports before treating their normalized event coverage as complete.
