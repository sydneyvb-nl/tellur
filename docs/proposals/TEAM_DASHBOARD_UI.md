...
| A12 | File source contract (opt-in): Git-provider link/fetch **or** `GET .../repos/{repo}/blob/{git_blob_sha}` | File view (full-source mode) | Persisted attribution has no source text. **Shipped:** per-repo `repo_source` with `template` (provider web-view deep link) + `raw_template` (raw bytes); the file view deep-links each range and, via the opt-in **Show source** toggle, fetches raw bytes **client-side** from the provider to render an inline gutter (CSP `connect-src` all-lists raw.githubusercontent.com/gitlab.com/bitbucket.org). The hub stores/serves no source. The hub-proxy `/blob` variant remains intentionally unbuilt (only needed for private repos without provider CORS; would need SSRF allow-listing + secret redaction). |
