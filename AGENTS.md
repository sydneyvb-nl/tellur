...
| Unified setup (`tellur setup`) | `crates/cli/src/onboarding.rs` + `setup.rs` + `connect.rs` + `service.rs` | Machine-wide onboarding: idempotently generates Codex/Claude/Gemini/Antigravity hooks, MCP/editor settings, optional Team Hub device login, and background sync. Every configured agent/editor auto-initializes any Git repo on first use; `tellur init` and `hooks ingest --auto-init` share `ensure_repo_git_automation`, installing **chained, non-clobbering git hooks** (`post-commit` → commit-scoped `notes export`; `pre-push` → idempotent `notes install-config` + hub/notes push, guarded and best-effort). There is no repeated per-repo setup requirement; `.tellur/disable` opts a repo out. `setup update/status`, legacy `connect`, and granular `setup <tool>` remain recovery surfaces. The generated personal plugin is the sole Codex hook owner. **No `remote.push` refspec**. |
