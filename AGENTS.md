...
| Team/server hub (Tier 1) | `crates/server/` | `tellur-server` binary — **FSL-1.1-ALv2**, not Apache (own `LICENSE`, `publish = false`). Layered: `config`/`error`/`auth` (roles + Argon2id tokens)/`storage` (Store trait + SQLite: orgs/members/tokens/hash-chained audit)/`api` (deny-by-default `Principal` extractor + tenant-scoped handlers)/`app` (router). `main.rs` = `serve` + `admin` bootstrap CLI. B0–B1 done. See `docs/proposals/TEAM_SERVER_IMPLEMENTATION.md`. |
