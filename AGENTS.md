...
| Team/server hub (Tier 1) | `crates/server/` | `tellur-server` binary — **FSL-1.1-ALv2**, not Apache (own `LICENSE`, `publish = false`). Layered: `config`/`error`/`auth` (roles + Argon2id tokens)/`ratelimit` (fixed-window)/`storage` (Store trait + SQLite: orgs/members/tokens/hash-chained audit + repos/per-repo event chain; `chain.rs` is the shared tamper-evident hash-chain helper — head checkpoint + verify — used by both the audit and event chains)/`api` (deny-by-default `Principal` extractor, tenant-scoped handlers: ingest w/ redaction + read/report + policy + attribution + export endpoints incl. per-repo SLSA/SPDX via core generators)/`metrics` (`/metrics`)/`app` (router + body-limit). `main.rs` = `serve` + `admin` bootstrap CLI (incl. `set-policy`). Heavy ops (report/export) run via `spawn_blocking`. Packaging: `dist/docker/`. B0–B4 + B5 (metrics/offload/packaging) done; Postgres backend is a separate upcoming PR. |
