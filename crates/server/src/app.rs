...
pub fn build_router(state: AppState) -> Router {
    let router = Router::new()
        ...
        // Device authorization for CLI `tellur login` (RFC 8628-style). The two
        // `/v1/device/*` endpoints are unauthenticated (claim + poll); the
        // `/auth/device*` pages require a signed-in session to approve.
        .route("/v1/device/authorize", post(crate::api::device_authorize))
        .route("/v1/device/token", post(crate::api::device_token))
        .route("/auth/device", get(crate::api::device_page))
        .route("/auth/device/decision", post(crate::api::device_decision))
        .route(
            "/webhook/github",
            post(crate::github_webhook::github_webhook),
        )
